package com.aljis.api_gateway.routes;
...
public class Routes {
    ...
    public RouterFunction<ServerResponse> productServiceSwaggerRoute() {
        URI fallbackUri = URI.create("forward:/fallbackRoute");
        return GatewayRouterFunctions.route("product_service_swagger")
                .route(RequestPredicates.path("/aggregate/product_service/v3/api-docs"), HandlerFunctions.http("http://localhost:8080"))
                .filter(CircuitBreakerFilterFunctions.circuitBreaker("productServiceSwaggerCircuitBreaker", fallbackUri))
